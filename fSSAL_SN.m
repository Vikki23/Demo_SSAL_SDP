function [varargout] = fSSAL_SN(varargin)
% 
% Semi-Supervised Learing (SSL): self-learning
% Active Learning (AL): BT - sort the BT values in ascending order
% Spatial Constraint: first the superpixel map, then the 4-connected neighbor
%
% [mlr_results,mpm_results,train] = fSSAL_SN(img,train,test,gt_map,...
...
%
%    2- Use the obtained classification map and the superpixel or 
%    neightborhood based spatial information to generate the candidate set 
%    whose samples are of high confidence. 
%
...
%
% mpm_results - MPM classification results : 9 \times 1 struct,
%               map        -   classification map
%               OA         -   classification overall accuracy
%               AA         -   classification average accuracy
%               kappa      -   classification kappa statistic
...
%                              samples per iteration
%               startpoint -   the iteration where the DP augmentation starts
%
...
for iter = 1:tot_sim
    ...
    % gather the confident candidate set
    SSLSet = [];
    % before the watershred: S strategy
    if startpoint == tot_sim
        ...
        for i = 1:tsize_real
            ...
            % delete the selected training samples from the SSL candidates 
    ...
    else  % after the watershred: N strategy
        % find the 4-neighbor samples of a given labeled sample 
        for i = 1:size(train,2)
            Neighbor = [];
            if train(1,i) == 1 % top left corner
                Neighbor = [train(1,i)+1 train(1,i)+img.size(1)];
            elseif train(1,i) == img.size(1) % down left corner
                Neighbor = [train(1,i)-1 train(1,i)+img.size(1)];
            elseif train(1,i) == img.size(1)*(img.size(2)-1)+1 % top right corner
                Neighbor = [train(1,i)+1 train(1,i)-img.size(1)];
            elseif train(1,i) == img.size(1)*img.size(2) % down right corner
                Neighbor = [train(1,i)-1 train(1,i)-img.size(1)];
            elseif ismember(train(1,i),[2:(img.size(1)-1)]) % left edge
                Neighbor = [train(1,i)-1 train(1,i)+1 train(1,i)+img.size(1)];
            elseif ismember(train(1,i),[(img.size(1)*(img.size(2)-1)+2):(img.size(1)*img.size(2)-1)]) % right edge
                Neighbor = [train(1,i)-1 train(1,i)+1 train(1,i)-img.size(1)];
            elseif ismember(train(1,i),img.size(1)*[1:img.size(2)-2]+1) % top edge
                Neighbor = [train(1,i)+1 train(1,i)-img.size(1) train(1,i)+img.size(1)];
            elseif ismember(train(1,i),img.size(1)*[2:img.size(2)-1]) % down edge
                Neighbor = [train(1,i)-1 train(1,i)-img.size(1) train(1,i)+img.size(1)];
            else
                Neighbor = [train(1,i)-1 train(1,i)+1 train(1,i)-img.size(1) train(1,i)+img.size(1)];
            end
            Neighbor(2,:) = mpm_results.map(iter,Neighbor);
            
            % remove the neighbor samples which have been already in the training set
            delNeighborInd = (ismember(Neighbor(1,:),train(1,:))==1);
            Neighbor(:,delNeighborInd) = [];
            
            % pick confident neighbor samples out
            CNeighborInd = (Neighbor(2,:)==train(2,i));
            CNeighbor = Neighbor(:,CNeighborInd);
            % remove the existing confident neighbor samples (already in SSLSet)
            if ~isempty(SSLSet)
                delCNeighborInd = (ismember(CNeighbor(1,:),SSLSet(1,:))==1); 
                CNeighbor(:,delCNeighborInd) = [];
            end

            % refresh the confident neighbor pool
            SSLSet = [SSLSet CNeighbor];
        end
    end
...
varargout(1) = {mlr_results}; % train_size OA AA kappa CA sslbt map startpoint
